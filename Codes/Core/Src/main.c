...
int main(void)
{
  ...
  uint32_t ui_tick = HAL_GetTick();
  while (1)
  {
    if (g_audio_half_ready != 0U)
    {
      g_audio_half_ready = 0U;
      AudioEngine_ProcessHalf();
    }

    if (g_audio_full_ready != 0U)
    {
      g_audio_full_ready = 0U;
      AudioEngine_ProcessFull();
    }
...
{
  (void)hadc;
  g_audio_half_ready = 1U;
}

void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
  (void)hadc;
  g_audio_full_ready = 1U;
}
/* USER CODE END 4 */
